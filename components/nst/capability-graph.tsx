...
const NODES: NodeData[] = [
  {
    ...
    z3d: 0.5,
    icon: Users,
    metric: "12,480+ Personnel",
    description: "Verified field commanders, tactical specialists, and analysts",
  ...
  },
  {
    ...
    z3d: -0.2,
    icon: Shield,
    metric: "100% Assurance",
    description: "National doctrine, sovereign defense protocols, and command readiness",
    status: "Verified Council"
  },
  {
    id: "knowledge",
    label: "KNOWLEDGE",
    ...
    icon: BookOpen,
    metric: "140+ Reports",
    description: "Monographs, classified curricula, and open-source intelligence feeds",
  ...
  },
  {
    ...
    icon: Building2,
    metric: "210+ Entities",
    description: "Defense manufacturers, sovereign departments, and research institutions",
    status: "Sovereign Tier"
  },
  {
    id: "opportunities",
    label: "OPPORTUNITIES",
    ...
    z3d: -0.4,
    icon: Compass,
    metric: "78 Active Missions",
    description: "Field deployments, research fellowships, and drone operations",
    status: "Open Briefs"
  }
];
...
export function CapabilityGraph() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Draw 3D grid rings
      ctx.strokeStyle = "rgba(217, 83, 37, 0.06)";
      ctx.lineWidth = 1;
      ...
      particles.forEach((pt) => {
        ...
        const alpha = Math.max(0.1, Math.min(0.5, 1 - p.z / 6));
      ...
      const connections: [number, number][] = [
        ...
        [4, 1], // opportunities -> strategy (inner chord)
        [3, 1], // organisations -> strategy (inner chord)
        [0, 2], // people -> knowledge (transverse chord)
      ];

      // Draw translucent mesh fill
      ctx.beginPath();
      ...
      meshGrad.addColorStop(0, "rgba(217, 83, 37, 0.12)");
      meshGrad.addColorStop(1, "rgba(217, 83, 37, 0.02)");
      ...
      connections.forEach(([i, j]) => {
        ...
        ctx.strokeStyle = "rgba(217, 83, 37, 0.72)";
        ctx.lineWidth = 1.2;
        ctx.beginPath();
        ...
        // Pulsing data beam on edges
        const pulseT = ((Date.now() * 0.0014 + i * 0.3) % 1);
      ...
      projectedNodes.forEach((node) => {
        ...
        ctx.lineWidth = 2;
        ctx.beginPath();
        ctx.arc(p.px, p.py, isCurrent ? 6 : 4.5, 0, Math.PI * 2);
        ctx.stroke();

        // Inner solid core
        ctx.fillStyle = isCurrent ? "#ffffff" : "#d95325";
        ctx.beginPath();
        ctx.arc(p.px, p.py, isCurrent ? 3 : 2, 0, Math.PI * 2);
        ctx.fill();

        // Label matching screenshot format
        ctx.font = "600 9.5px 'Inter', sans-serif";
        ctx.fillStyle = isCurrent ? "#ffffff" : "#9ba1a9";
  ...
  return (
    <div className="relative w-full max-w-[500px] mx-auto group">
      ...
      <div className="relative aspect-[1/0.82] w-full border border-[#3b414a] bg-[#181c23] overflow-hidden shadow-2xl">
        ...
        <div className="network-grid-bg absolute inset-0 pointer-events-none" />
        ...
        <AnimatePresence>
          {activeNode && (
            <motion.div
              ...
              className="absolute bottom-3 left-3 right-3 bg-[#111419]/95 backdrop-blur-md border border-[#d95325]/40 p-3 z-30 shadow-2xl"
            >
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
                  <div className="w-4 h-4 rounded bg-[#d95325]/15 flex items-center justify-center text-[#d95325]">
                ...
                <span className="text-[9px] font-mono font-semibold px-2 py-0.5 bg-[#d95325]/20 text-[#d95325] border border-[#d95325]/30">
                  {activeNode.status}
                </span>
              </div>
              <p className="text-[11px] text-[#c5c9ce] leading-relaxed mb-1.5 font-sans">
                {activeNode.description}
              </p>
              <div className="flex items-center justify-between pt-1.5 border-t border-[#3b414a]/80 text-[10px] font-mono text-[#9299a2]">
                <span>Verified Metric:</span>
                <span className="text-white font-bold">{activeNode.metric}</span>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
      ...
      {/* Caption matching screenshot exactly */}
